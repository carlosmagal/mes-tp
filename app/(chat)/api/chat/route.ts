import { appendClientMessage, createDataStream, UIMessage } from "ai";
...
import { FlowiseResponse } from "@/lib/types/flowise";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!flowiseResponse.ok) {
      ...
      throw new Error(
        `Flowise API request failed with status ${flowiseResponse.status}: ${errorBody}`
      );
    }

    const responseData: FlowiseResponse = await flowiseResponse.json();
    const text = responseData.text ?? "";
    const agentReasoning = responseData.agentReasoning ?? [];

    console.log(responseData.agentReasoning);
